...
class GRPOLanguageTrainerModule(TrainerModule, LoggerMixin):
    ...
    def _initialize_generation_config(self):
        """Initialize generation configuration for standard training."""
        if self.use_vllm:
            return
    ...
    def _process_inputs(self, inputs, with_template=True, for_training=False):
        """Process inputs for both vLLM and standard training."""
    ...
    def generate(
        ...
        Supports both vLLM and standard generation.
        """
        if self.use_vllm:
    ...
    def train(
        ...
        if self.use_vllm:
            print("Training is not supported in vLLM mode.")
            return
